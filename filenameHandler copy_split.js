...
function checkFilenameAgainstHeader (type, filename, comment, deviceID) {
    ...
    if (type === EXPAND) {
        ...
        originalTimestring = matches[2];

        existingPrefix = matches[1] ? matches[1] : '';

        if (type === SYNC) devicePrefix = existingPrefix;
    ...
    if ((type === EXPAND || type === SYNC) && devicePrefix) {

        const prefix = devicePrefix.substring(0, devicePrefix.length - 1);
    ...
    if (type === SPLIT || type === EXPAND || type === SYNC) {
        ...
        const expectedTimestring = type === EXPAND && expandHasDatePrefix === false ? dateMatches[1] + dateMatches[2] + dateMatches[3] : dateMatches[6] + dateMatches[5] + dateMatches[4] + '_' + dateMatches[1] + dateMatches[2] + dateMatches[3];
